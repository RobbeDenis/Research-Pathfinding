...
void App_PathfindingAStar::Update(float deltaTime)
{
	...
	if (m_UpdatePath 
		...
		/*&& startPathIdx != endPathIdx*/) // Dont include otherwise path doesnt update if start is end
	{
		auto startNode = m_pGridGraph->GetNode(startPathIdx);
		auto endNode = m_pGridGraph->GetNode(endPathIdx);
		m_vSearchedNodes.clear();
		...
		case PathfinderType::aStar:
		{
			...
			m_vPath = pathfinder.FindPath(startNode, endNode, m_vSearchedNodes);
			break;
		}
		...
		m_UpdatePath = false;
		std::cout << "New Path Calculated in " << elapsedSec.count() * 100.f << " ms" <<std::endl;
	}
}
...
void App_PathfindingAStar::Render(float deltaTime) const
{
	...
	if (m_vSearchedNodes.size() > 0)
	{
		for (Elite::GridTerrainNode* node : m_vSearchedNodes)
		{
			DEBUGRENDERER2D->DrawCircle(m_pGridGraph->GetNodeWorldPos(node->GetIndex()), 1.f, { 1.f,1.f,1.f }, 0.1f);
		}
	}
}
...
#pragma region UI
	//UI
	{
		...
		if (ImGui::Combo("", &m_PathfinderIndex, "BFS\0AStar\0Dijkstra", 2))
		{
			switch (m_PathfinderIndex)
			{
			case 0:
				m_PathfinderType = PathfinderType::bfs;
				break;
			case 1:
				m_PathfinderType = PathfinderType::aStar;
				break;
			case 2:
				m_PathfinderType = PathfinderType::dijkstra;
				break;
			}
			m_UpdatePath = true;
		}
