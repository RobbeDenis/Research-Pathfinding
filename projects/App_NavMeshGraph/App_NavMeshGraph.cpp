...
std::vector<Elite::Vector2> App_NavMeshGraph::FindPath(Elite::Vector2 startPos, Elite::Vector2 endPos)
{
	...
	//Copy the graph
	std::shared_ptr<IGraph<NavGraphNode, GraphConnection2D>> copyGraph = m_pNavGraph->Clone();
	
	//Create extra node for the Start Node (Agent's position)
	//Create extra node for the End Node
	NavGraphNode* startNode = new NavGraphNode(copyGraph->GetNextFreeNodeIndex(), invalid_node_index, startPos);
	int startIdx = copyGraph->AddNode(startNode);
	NavGraphNode* endNode = new NavGraphNode(copyGraph->GetNextFreeNodeIndex(), invalid_node_index, endPos);
	int endIdx = copyGraph->AddNode(endNode);
	
	for (int i = 0; i < startTriangle->metaData.IndexLines.size(); i++)
	{
		int nodeIdx = m_pNavGraph->GetNodeIdxFromLineIdx(startTriangle->metaData.IndexLines[i]);

		if(nodeIdx != invalid_node_index)
			copyGraph->AddConnection(new GraphConnection2D(startNode->GetIndex(), nodeIdx, Distance(m_pNavGraph->GetNode(nodeIdx)->GetPosition(), startNode->GetPosition())));
	}

	for (int i = 0; i < endTriangle->metaData.IndexLines.size(); i++)
	{
		int nodeIdx = m_pNavGraph->GetNodeIdxFromLineIdx(endTriangle->metaData.IndexLines[i]);

		if (nodeIdx != invalid_node_index)
			copyGraph->AddConnection(new GraphConnection2D(endNode->GetIndex(), nodeIdx, Distance(m_pNavGraph->GetNode(nodeIdx)->GetPosition(), endNode->GetPosition())));
	}
	
	//Run A star on new graph
	auto pathfinder = AStar<NavGraphNode, GraphConnection2D>(copyGraph.get(), HeuristicFunctions::Chebyshev);
	std::vector<NavGraphNode*> nodes = pathfinder.FindPath(copyGraph.get()->GetNode(startIdx), copyGraph.get()->GetNode(endIdx), vector<NavGraphNode*>{});

	//Fill the debug nodes with the right positions
