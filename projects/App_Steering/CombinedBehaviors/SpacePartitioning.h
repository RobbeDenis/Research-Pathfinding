...
public:
	...
	void ResetCellAgents();

	void RegisterNeighbors(const Elite::Vector2& targetPos, float queryRadius, float deltaT);
	const std::vector<SteeringAgent*> GetNeighbors() const { return m_Neighbors; }
	int GetNrOfNeighbors() const { return m_NrOfNeighbors; }

	void RenderCells(SteeringAgent* pAgent, float queryRadius)const;
